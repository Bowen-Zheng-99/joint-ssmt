...
#
# WITH WINSORIZATION:
#   - Caps extreme beta magnitudes per frequency band
#   - Preserves phase while limiting outliers from low-power bands
#   - Applied after warmup and after each refresh
#
...
from joint_ssmt.joint_inference_core_trial_fast import joint_kf_rts_moments_trials_fast
from joint_ssmt.beta_sampler_trials_jax import TrialBetaConfig, gibbs_update_beta_trials_shared_Xrtp_vectorized
from joint_ssmt.polyagamma_jax import sample_pg_saddle_single
...
def _winsorize_beta_samples(
    beta_samples: np.ndarray,
    J: int,
    percentile: float = 95.0,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Winsorize beta samples by capping magnitude while preserving phase.
    
    For each frequency band j, finds the percentile threshold across all 
    samples and units, then scales any (β_R, β_I) pair whose magnitude 
    exceeds the threshold.
    
    This prevents outlier beta values from frequency bands with low power
    while preserving the phase information (atan2(β_I, β_R) unchanged).
    
    Args:
        beta_samples: (n_samples, S, P) where P = 1 + 2*J
            MCMC samples of beta parameters [intercept, β_R, β_I]
        J: int
            Number of frequency bands
        percentile: float
            Upper percentile for magnitude cap (computed per frequency 
            across all samples and units)
    
    Returns:
        beta_winsorized: (n_samples, S, P) array
            Winsorized samples with extreme magnitudes capped
        thresholds: (J,) array
            The threshold used for each frequency band (for diagnostics)
    """
    n_samples, S, P = beta_samples.shape
    
    # Make a copy to avoid modifying input
    beta_winsorized = beta_samples.copy()
    
    # Extract real and imaginary parts: (n_samples, S, J)
    beta_R = beta_winsorized[:, :, 1:1+J]
    beta_I = beta_winsorized[:, :, 1+J:1+2*J]
    
    # Compute magnitude per sample: (n_samples, S, J)
    magnitude = np.sqrt(beta_R**2 + beta_I**2)
    
    # For each frequency, find threshold across all samples and units
    # Shape: (J,)
    thresholds = np.percentile(magnitude, percentile, axis=(0, 1))
    
    # Compute scale factor: threshold / magnitude (capped at 1)
    # Same scale applied to both β_R and β_I to preserve phase
    # Shape: (n_samples, S, J)
    scale = np.minimum(1.0, thresholds[np.newaxis, np.newaxis, :] / 
                       np.maximum(magnitude, 1e-10))
    
    # Apply scaling
    beta_winsorized[:, :, 1:1+J] = beta_R * scale
    beta_winsorized[:, :, 1+J:1+2*J] = beta_I * scale
    
    # Count how many samples were clipped
    n_clipped = np.sum(scale < 1.0)
    n_total = n_samples * S * J
    
    return beta_winsorized, thresholds


def _print_winsorization_diagnostics(
    thresholds: np.ndarray,
    freqs_hz: Sequence[float],
    n_clipped: int,
    n_total: int,
) -> None:
    """Print winsorization diagnostics."""
    pct_clipped = 100.0 * n_clipped / n_total if n_total > 0 else 0
    logger.info(f"[WINSORIZE] Clipped {n_clipped}/{n_total} ({pct_clipped:.1f}%) sample-unit-freq triplets")
    logger.info(f"[WINSORIZE] Threshold range: [{thresholds.min():.4f}, {thresholds.max():.4f}]")
...
def _wald_test_band_selection(
    ...
    if verbose:
        ...
        total_pairs = S * J
        logger.info(f"[WALD] Band selection (α={alpha}, burn-in={burn_in_frac}):")
        logger.info(f"[WALD]   Using {n_post} post-burn-in samples")
        logger.info(f"[WALD]   Significant (neuron,band) pairs: {n_sig_pairs}/{total_pairs}")
        logger.info(f"[WALD]   Bands with ANY significant coupling: {n_sig_bands}/{J}")
        ...
        if freqs_hz is not None:
            ...
            if sig_freqs:
                logger.info(f"[WALD]   Coupled bands (joint KF): {', '.join(sig_freqs)}")
            if nonsig_freqs and len(nonsig_freqs) <= 10:
                logger.info(f"[WALD]   Uncoupled bands (LFP-only): {', '.join(nonsig_freqs)}")
            elif nonsig_freqs:
                logger.info(f"[WALD]   Uncoupled bands (LFP-only): {len(nonsig_freqs)} bands")
...
def _zero_nonsignificant_beta_per_pair(
    ...
    logger.info(f"[GATING] β masking: {n_sig}/{S*J} pairs significant, {n_zeroed} zeroed")
...
def _apply_true_band_gating(
    ...
    logger.info(f"[GATING] State gating: {n_coupled} bands from joint KF, {n_uncoupled} bands from LFP-only")
...
def run_joint_inference_trials_hier(
    ...
    *,
    ...
    WITH WINSORIZATION:
    - Caps extreme beta magnitudes per frequency band
    - Preserves phase while limiting outliers
    ...
    logger.info("[HIER-JOINT] Starting hierarchical joint inference...")
    logger.info(f"[HIER-JOINT] freeze_beta0 = {config.freeze_beta0}")
    logger.info(f"[HIER-JOINT] standardize_latents = {config.standardize_latents}")
    logger.info(f"[HIER-JOINT] use_beta_shrinkage = {config.use_beta_shrinkage}")
    logger.info(f"[HIER-JOINT] use_wald_band_selection = {config.use_wald_band_selection}")
    logger.info(f"[HIER-JOINT] use_winsorization = {config.use_winsorization}")
    if config.use_winsorization:
        logger.info(f"[HIER-JOINT]   percentile = {config.winsorize_percentile}")
        logger.info(f"[HIER-JOINT]   after_warmup = {config.winsorize_after_warmup}")
        logger.info(f"[HIER-JOINT]   after_refresh = {config.winsorize_after_refresh}")
    logger.info(f"[HIER-JOINT] trace_thin = {config.trace_thin}")
    if config.use_wald_band_selection:
        logger.info(f"[HIER-JOINT] TRUE GATING ENABLED:")
        logger.info(f"[HIER-JOINT]   - Pair-specific β masking")
        logger.info(f"[HIER-JOINT]   - Uncoupled bands keep LFP-only estimates")
    ...
    P = 1 + 2 * J
    logger.info(f"[HIER-JOINT] Data dimensions: R={R}, S={S}, J={J}, M={M}, T={T_f}, Rlags={Rlags}")
    ...
    logger.info("[HIER-JOINT] Running hierarchical EM warm start...")
    ...
    logger.info("[HIER-JOINT] Hierarchical EM complete")
    logger.info(f"[HIER-JOINT] θ_X: λ range [{theta_X.lam.min():.2f}, {theta_X.lam.max():.2f}]")
    logger.info(f"[HIER-JOINT] θ_D: λ range [{theta_D.lam.min():.2f}, {theta_D.lam.max():.2f}]")
    logger.info(f"[HIER-JOINT] θ_Z (combined): λ range [{theta_Z.lam.min():.2f}, {theta_Z.lam.max():.2f}]")

    # 1) Upsample X and D to fine grid
    from joint_ssmt.upsample_ct_hier_fine import upsample_ct_hier_fine
    logger.info("[HIER-JOINT] Upsampling EM latents to fine grid...")
    ...
    logger.info(f"[HIER-JOINT] Stored EM estimates as LFP-only reference (shape: {Z_fine_em.shape})")
    ...
    if config.standardize_latents:
        ...
        logger.info(f"[HIER-JOINT] Latent scale factors (min={latent_scale_factors.min():.4f}, "
              f"max={latent_scale_factors.max():.4f})")
    else:
        latent_scale_factors = np.ones(2 * J, dtype=float)
        logger.info("[HIER-JOINT] Latent standardization DISABLED")
    ...
    trace.winsorize_thresholds = []  # Store winsorization thresholds
    ...
    logger.info("[HIER-JOINT] Starting warmup loop...")
    ...
    logger.info(f"[HIER-JOINT] Warmup complete ({len(beta_hist)} samples stored)")

    # =================================================================
    # WINSORIZATION AFTER WARMUP
    # =================================================================
    if config.use_winsorization and config.winsorize_after_warmup:
        logger.info(f"[HIER-JOINT] Applying winsorization to warmup samples (percentile={config.winsorize_percentile})")
        beta_hist, thresholds = _winsorize_beta_samples(
            beta_hist, J, percentile=config.winsorize_percentile
        )
        trace.winsorize_thresholds.append(thresholds)
        n_clipped = np.sum(np.sqrt(beta_hist[:, :, 1:1+J]**2 + beta_hist[:, :, 1+J:1+2*J]**2) >= thresholds[np.newaxis, np.newaxis, :] - 1e-8)
        _print_winsorization_diagnostics(thresholds, all_freqs, n_clipped, beta_hist.shape[0] * S * J)
    ...
    if config.freeze_beta0:
        ...
        logger.info(f"[HIER-JOINT] Froze β₀ to median of last {w} warmup samples")
    else:
    ...
    gc.collect()
    logger.info(f"[HIER-JOINT] Warmup trace stored, memory freed")

    # =================================================================
    # WALD TEST FOR BAND SELECTION (after warmup)
    # =================================================================
    if config.use_wald_band_selection:
        logger.info("[HIER-JOINT] Performing Wald test for band selection...")
        ...
        logger.info(f"[HIER-JOINT] {n_significant}/{J} bands will use joint KF")
        logger.info(f"[HIER-JOINT] {J - n_significant}/{J} bands will keep LFP-only (EM) estimates")
    else:
        ...
        logger.info("[HIER-JOINT] Wald test disabled - all bands use joint KF")

    # 5) Refresh passes using NON-HIERARCHICAL KF (treats Z = X + D jointly)
    sidx = StateIndex(J, M)
    logger.info(f"[HIER-JOINT] Starting {config.n_refreshes} refresh passes...")
    
    for rr in range(config.n_refreshes):
        logger.info(f"[HIER-JOINT] Refresh {rr + 1}/{config.n_refreshes}")
        ...
        # =================================================================
        # WINSORIZATION AFTER REFRESH
        # =================================================================
        if config.use_winsorization and config.winsorize_after_refresh:
            logger.info(f"[HIER-JOINT] Applying winsorization to refresh {rr+1} samples")
            inner_beta_hist, thresholds = _winsorize_beta_samples(
                inner_beta_hist, J, percentile=config.winsorize_percentile
            )
            trace.winsorize_thresholds.append(thresholds)
        ...
        # -----------------------------------------------------------------
        logger.info(f"[HIER-JOINT] Refresh {rr + 1}: Pass 1 - estimating X (pooled)")
        ...
        # -----------------------------------------------------------------
        logger.info(f"[HIER-JOINT] Refresh {rr + 1}: Pass 2 - estimating Z_r (per-trial)")
        ...
        logger.info(f"[HIER-JOINT] X/D decomposition: D variance fraction = {D_fraction:.1f}%")
        
        # Debug: print cross-trial variability
        Z_trial_std = np.std(Z_fine_gated, axis=0).mean()
        logger.info(f"[HIER-JOINT] Z cross-trial std (mean): {Z_trial_std:.6f}")
        ...
        logger.info(f"[HIER-JOINT] Refresh {rr + 1} complete (trace has {len(trace.beta)} samples)")
    ...
    # =================================================================
    # STORE STANDARDIZED β (before rescaling)
    # =================================================================
    beta_final = np.asarray(beta)
    trace.beta_standardized = beta_final.copy()  # Final point estimate
    trace.beta_trace_standardized = [b.copy() for b in trace.beta]  # Full trace
    ...
    logger.info("[HIER-JOINT] Inference complete")
    logger.info(f"[HIER-JOINT] Total trace samples: {len(trace.beta)}")
    logger.info(f"[HIER-JOINT] Final D variance fraction: {D_fraction:.1f}%")
    if config.use_beta_shrinkage:
        logger.info(f"[HIER-JOINT] Beta shrinkage was ENABLED")
    if config.use_winsorization:
        logger.info(f"[HIER-JOINT] Winsorization was ENABLED (percentile={config.winsorize_percentile})")
    if config.use_wald_band_selection:
        logger.info(f"[HIER-JOINT] TRUE GATING was ENABLED:")
        logger.info(f"[HIER-JOINT]   - {significant_mask.sum()}/{J} bands used joint KF")
        logger.info(f"[HIER-JOINT]   - {J - significant_mask.sum()}/{J} bands kept LFP-only")
