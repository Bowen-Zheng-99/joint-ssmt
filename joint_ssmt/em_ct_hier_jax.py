...
"""
Hierarchical CT-SSMT EM with optional constant-mean OU.

NEW: use_mean_reversion parameter (default=True)
  - True: X reverts to μ = mean(Y_pool) per chain (better for short windows)
  - False: X reverts to 0 (original behavior)
  - D always reverts to 0 (deviations are zero-mean by construction)
"""
...
class EMHierResult:
    ...
    # NEW: mean reversion targets (for upsampler consistency)
    mu_X: Optional[jnp.ndarray] = None  # (J,M) complex, None if use_mean_reversion=False
    mu_D: Optional[jnp.ndarray] = None  # (J,M) complex, always 0 for D
...
def _rtss_ou_single_taper_jax(Y_m, phi_val, q_val, R_val, z0, P0):
    """Scalar complex OU RTS for one taper, with user-supplied z0,P0. Zero-mean OU."""
    K = Y_m.shape[0]
...
def _rtss_ou_jax(Y, lam, sig_v, sig_eps, db, z0_jm, P0_jm):
    """
    RTS smoother with user initial state per chain. Zero-mean OU.
...
@jax.jit
def _rtss_ou_jax_with_mean(Y, lam, sig_v, sig_eps, db, z0_jm, P0_jm, mu_jm):
    """
    RTS smoother with constant-mean OU.
    
    Model: (Z - μ)_{k+1} = φ·(Z - μ)_k + η
    
    Implementation: center Y by μ, run zero-mean RTS, add μ back.
    
    Y: (J,M,K), lam/sig_v: (J,M), sig_eps: (J,M) or (1,M), 
    z0_jm/P0_jm: (J,M), mu_jm: (J,M) complex mean
    """
    # Center observations
    Y_centered = Y - mu_jm[:, :, None]  # (J,M,K)
    
    # Center initial state
    z0_centered = z0_jm - mu_jm  # (J,M)
    
    # Run zero-mean RTS on centered data
    xs_centered, Ps, Csum, Rprev, Rnext, xp, Pp, E_YY, E_YZ, E_ZZ, E_dZ = \
        _rtss_ou_jax(Y_centered, lam, sig_v, sig_eps, db, z0_centered, P0_jm)
    
    # Add mean back to smoothed estimates
    xs = xs_centered + mu_jm[:, :, None]  # (J,M,K)
    
    # Note: Csum, Rprev, Rnext are computed on centered data, which is correct
    # for M-step (we're estimating dynamics of the centered process)
    
    return xs, Ps, Csum, Rprev, Rnext, xp, Pp, E_YY, E_YZ, E_ZZ, E_dZ
...
@partial(jax.jit,
         static_argnames=("max_iter","obs_noise_shared","no_pool_iters",
                          "enable_logging","print_every","use_mean_reversion"))
def _em_ct_hier_loop(
    ...
    enable_logging: bool,
    print_every: int,
    use_mean_reversion: bool,                     # NEW: toggle constant-mean OU
):
    R, M, J, K = Y.shape
    ...
    # NEW: Compute mean for X from trial-averaged Y (constant across iterations)
    # For D, mean is always 0 (deviations from X)
    Y_jmk = jnp.swapaxes(jnp.mean(Y, axis=0), 0, 1)  # (J,M,K)
    mu_X = jnp.mean(Y_jmk, axis=2)  # (J,M) - time-average per chain
    mu_D = jnp.zeros((J, M), dtype=Y.dtype)  # D is zero-mean
    ...
    def body_fun(st):
        ...
        def run_one_D(Y_rjmK, sig_eps_r, z0_rjm, P0_rjm):
            Y_res = Y_rjmK - X_mean_s  # Residual = Y - X
            # D is always zero-mean, so use original _rtss_ou_jax
        ...
        xs_D_r, Ps_D_r, Csum_D_r, Rprev_D_r, Rnext_D_r, xp_D_r, Pp_D_r, *_ = \
            vmap(run_one_D, in_axes=(0, 0, 0, 0))(Y_rjmk, sig_eps_trials, st["x0_D"], st["P0_D"])
        ...
        # NEW: Use constant-mean or zero-mean based on flag
        if use_mean_reversion:
            xs_X, Ps_X, Csum_X, Rprev_X, Rnext_X, xp_X, Pp_X, *_ = \
                _rtss_ou_jax_with_mean(Y_pool, lam_X_s, sigv_X_s, sig_pool, db, 
                                       st["x0_X"], st["P0_X"], st["mu_X"])
        else:
            xs_X, Ps_X, Csum_X, Rprev_X, Rnext_X, xp_X, Pp_X, *_ = \
                _rtss_ou_jax(Y_pool, lam_X_s, sigv_X_s, sig_pool, db, 
                             st["x0_X"], st["P0_X"])
        ...
        # Note: Csum/Rprev/Rnext are computed on centered data when use_mean_reversion=True
        # This is correct - we estimate dynamics of (Z - μ), not Z
        ...
        return {
            ...
            "mu_X": st["mu_X"], "mu_D": st["mu_D"],  # mu is constant
...
def em_ct_hier_jax(
    ...
    *,
    ...
    tol_rel: float = 1e-7,             # relative ΔQ tolerance
    ...
    no_pool_iters: int = 5,
    rho_timescale: float = 0,
    q_floor_abs: float = 1e-8,
    log_every: int | None = None,
    use_mean_reversion: bool = False,   # NEW: default True for better short-window behavior
) -> EMHierResult:
    """
    ...
    Parameters
    ----------
    use_mean_reversion : bool, default=True
        If True, X reverts to μ = mean(Y_pool) per (j,m) chain (constant-mean OU).
        If False, X reverts to 0 (original zero-mean OU).
        D always reverts to 0 (deviations are zero-mean by construction).

    Notes
    -----
    - Noise M-step uses SMOOTHED moments.
    - Q monitor uses smoothed obs residuals.
    ...
    (lam_X, sigv_X, lam_D, sigv_D, sig2_eps_jmr,
     ...
     mu_X, mu_D) = _em_ct_hier_loop(
        ...
        use_mean_reversion=bool(use_mean_reversion),
    )
    ...
    return EMHierResult(
        ...
        x0_X=x0_X, P0_X=P0_X,
        x0_D=x0_D, P0_D=P0_D,
        mu_X=mu_X if use_mean_reversion else None,
        mu_D=mu_D if use_mean_reversion else None,
    )