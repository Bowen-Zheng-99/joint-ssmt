...
from joint_ssmt.joint_inference_core import joint_kf_rts_moments
# Standard PG-Gibbs sampler for β/γ
...
def _standardize_latents(
    ...
    Use the SAME scale factor for Re and Im of each frequency
    to preserve phase! Different scaling rotates the phase.
    """
    R, T, twoJ = lat_reim_RTP.shape
    J = twoJ // 2  # Number of frequency bands
    ...
    if scale_factors is None:
        ...
        for j in range(J):
            # Get Re and Im for this frequency
            re_j = lat_reim_RTP[:, :, j]
            im_j = lat_reim_RTP[:, :, J + j]
            
            # Compute scale from complex AMPLITUDE (preserves phase)
            amplitude_rms = np.sqrt(np.mean(re_j**2 + im_j**2))
            scale_j = max(amplitude_rms, min_std)

            # Use SAME scale for both Re and Im!
            scale_factors[j] = scale_j
            scale_factors[J + j] = scale_j
...
def _select_significant_neurons(
    ...
    if verbose:
        ...
        logger.info(f"[WALD] Neuron selection: {n_sig}/{S} neurons have significant coupling")

        if n_sig < S:
            excluded = np.where(~neuron_mask)[0]
            if len(excluded) <= 10:
                logger.info(f"[WALD]   Excluded neurons (no coupling): {excluded.tolist()}")
            else:
                logger.info(f"[WALD]   Excluded neurons: {len(excluded)} total")
...
def _zero_nonsignificant_beta_per_neuron(
    ...
    if verbose:
        ...
        n_sig = significant_pairs.sum()
        logger.info(f"[GATING] {n_sig}/{S*J} (neuron,band) pairs significant, {n_zeroed} zeroed")
...
class SingleTrialInferenceConfig:
    ...
    # Latent refresh control
    # When False, skip joint KF and keep EM (LFP-only) latent estimates
...
def run_joint_inference_single_trial(
    Y_cube: np.ndarray,              # (J, M, K)
    spikes_ST: np.ndarray,           # (S, T)
    H_STL: np.ndarray,               # (S, T, L)
    ...
    *,
    ...
    logger.info("[SINGLE-TRIAL] Starting joint inference...")
    logger.info(f"[SINGLE-TRIAL] freeze_beta0 = {config.freeze_beta0}")
    logger.info(f"[SINGLE-TRIAL] standardize_latents = {config.standardize_latents}")
    logger.info(f"[SINGLE-TRIAL] use_beta_shrinkage = {config.use_beta_shrinkage}")
    logger.info(f"[SINGLE-TRIAL] use_wald_band_selection = {config.use_wald_band_selection}")
    logger.info(f"[SINGLE-TRIAL] trace_thin = {config.trace_thin}")
    logger.info(f"[SINGLE-TRIAL] enable_latent_refresh = {config.enable_latent_refresh}")
    if config.use_wald_band_selection:
        logger.info(f"[SINGLE-TRIAL] Wald test band gating enabled (Pair-specific β masking)")
        logger.info(f"[SINGLE-TRIAL] Uncoupled bands will use LFP-only (EM) estimates")
    ...
    Rlags = H_STL.shape[-1]
    P = 1 + 2 * J
    logger.info(f"[SINGLE-TRIAL] Data dimensions: R={R}, S={S}, J={J}, M={M}, T={T_f}, Rlags={Rlags}")
    ...
    logger.info("[SINGLE-TRIAL] Running EM warm start...")
    res = em_ct_single_jax(Y=jnp.asarray(Y_cube), db=window_sec, **em_kwargs)
    theta = _theta_from_em_single(res, J=J, M=M)
    logger.info("[SINGLE-TRIAL] EM complete")
    logger.info(f"[SINGLE-TRIAL] θ: λ range [{theta.lam.min():.4f}, {theta.lam.max():.4f}]")

    # 1) Upsample Z to fine grid
    from joint_ssmt.upsample_ct_single_fine import upsample_ct_single_fine
    logger.info("[SINGLE-TRIAL] Upsampling EM latents to fine grid...")
    ...
    logger.info(f"[SINGLE-TRIAL] Stored EM estimates as LFP-only reference (shape: {Z_fine_em.shape})")
    ...
    if config.standardize_latents:
        ...
        logger.info(f"[SINGLE-TRIAL] Latent scale factors (min={latent_scale_factors.min():.4f}, "
              f"max={latent_scale_factors.max():.4f})")
    else:
        latent_scale_factors = np.ones(2 * J, dtype=float)
        logger.info("[SINGLE-TRIAL] Latent standardization DISABLED")
    ...
    logger.info("[SINGLE-TRIAL] Starting warmup loop...")
    ...
    logger.info(f"[SINGLE-TRIAL] Warmup complete ({len(beta_hist)} samples stored)")
    ...
    if config.freeze_beta0:
        ...
        logger.info(f"[SINGLE-TRIAL] Froze β₀ to median of last {w} warmup samples")
    else:
    ...
    gc.collect()
    logger.info(f"[SINGLE-TRIAL] Warmup trace stored, memory freed")

    # Wald test for band selection (after warmup)
    if config.use_wald_band_selection:
        logger.info("[SINGLE-TRIAL] Performing Wald test for band selection...")
        ...
        logger.info(f"[SINGLE-TRIAL] {n_significant}/{J} bands will use joint KF")
        logger.info(f"[SINGLE-TRIAL] {J - n_significant}/{J} bands will keep LFP-only (EM) estimates")
    else:
        ...
        logger.info("[SINGLE-TRIAL] Wald test disabled - all bands use joint KF")

    # 5) Refresh passes
    sidx = StateIndex(J, M)
    logger.info(f"[SINGLE-TRIAL] Starting {config.n_refreshes} refresh passes...")

    for rr in range(config.n_refreshes):
        logger.info(f"[SINGLE-TRIAL] Refresh {rr + 1}/{config.n_refreshes}")
        ...
        from joint_ssmt.utils_common import separated_to_interleaved
        beta_interleaved = separated_to_interleaved(beta_for_kf)

        # KF refresh (single-trial: no pooling)
        # Controlled by enable_latent_refresh config flag
        # When False, keep EM (LFP-only) latent estimates
        
        skip_kf = not config.enable_latent_refresh
        
        if skip_kf:
            # Keep EM estimates - don't run joint KF
            T0_new = min(T0, Z_fine_em.shape[1])
            Z_fine_gated = Z_fine_em[:, :T0_new, :].copy()
            Z_var_gated = Z_var_em[:, :T0_new, :].copy()
            logger.info(f"[SINGLE-TRIAL] Refresh {rr + 1}: SKIPPING KF (enable_latent_refresh=False)")
            logger.info(f"[SINGLE-TRIAL] Keeping LFP-only (EM) latent estimates")
        else:
            logger.info(f"[SINGLE-TRIAL] Refresh {rr + 1}: Running KF smoother...")
        ...
        logger.info(f"[SINGLE-TRIAL] Refresh {rr + 1} complete (trace has {len(trace.beta)} samples)")
    ...
    logger.info("[SINGLE-TRIAL] Inference complete")
    logger.info(f"[SINGLE-TRIAL] Total trace samples: {len(trace.beta)}")
    if config.use_wald_band_selection:
        logger.info(f"[SINGLE-TRIAL]   - {significant_mask.sum()}/{J} bands used joint KF")
        logger.info(f"[SINGLE-TRIAL]   - {J - significant_mask.sum()}/{J} bands kept LFP-only")
