...
def _m_step_dynamics(Csum_jm, Rprev_jm, Rnext_jm, K, db, update_lam, lam_old_jm):
    ...
    All inputs/outputs are (J, M) shaped.
    """
...
def _m_step_obs_noise(Y, xs, Ps):
    """M-step for observation noise (per taper)."""
...
def _compute_Q(E_YY, E_YZ, E_ZZ, E_dZ, Rbar, Qbar, K, J, M):
    """Compute Q function (log-likelihood)."""
...
def em_ct_single_jax(
    ...
    *,
    ...
    All dynamics parameters (lam, sigv) are (J, M) shaped.
    """
    Y = jnp.asarray(Y)
    J, M, K = Y.shape
