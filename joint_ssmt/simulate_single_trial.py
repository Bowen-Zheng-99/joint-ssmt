...
Uses the same coefficient layout and data generation logic as the trial-structured version.
Coefficient layout (SEPARATED): β = [β₀, βR₀..βR_{J-1}, βI₀..βI_{J-1}]
"""
...
class SingleTrialSimConfig:
    ...
    # Observation noise
    sigma_eps: np.ndarray = field(default_factory=lambda: np.array([10.0, 10.0, 10.0, 10.0], float))
...
def simulate_single_trial(cfg: SingleTrialSimConfig, seed: int | None = None) -> dict:
    ...
    Uses the same logic as the trial-structured version but without
    the trial dimension.
    ...
    # Ground truth amplitude at fine resolution
    A_t = np.abs(Z_fine)  # (J, T_fine)
    ...
    firing_rates = total_spikes / (T_fine * delta_spk)
    for s in range(S):
        print(f"  Unit {s} firing rate: {firing_rates[s]:.2f} Hz")
...
def main():
    ...
    parser.add_argument('--duration', type=float, default=300.0, help='Duration (sec)')
    ...
    parser.add_argument('--b0_mu', type=float, default=-4, help='Intercept mean')
