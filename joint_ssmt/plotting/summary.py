...
def plot_spectrogram(
    ...
    *,
    ...
    clip_percentile : (low, high)
        Colormap is clipped to these percentiles of the displayed power
        so that a few outlier bins do not flatten the dynamic range. Set
        to (0, 100) to disable.
    max_db_range : float
        When ``db_scale=True``, cap the displayed dynamic range so that
        ``vmin = vmax - max_db_range`` if the percentile-clipped range
        would otherwise be larger. Set to ``np.inf`` to disable.
    """
    _set_style()
    ...
    # Clip the colormap to the configured percentile range so that a few
    # outlier bins do not flatten the dynamic range.
    lo_pct, hi_pct = float(clip_percentile[0]), float(clip_percentile[1])
    finite = np.isfinite(power)
    if finite.any() and 0.0 <= lo_pct < hi_pct <= 100.0:
        vmin = float(np.percentile(power[finite], lo_pct))
        vmax = float(np.percentile(power[finite], hi_pct))
        if vmax <= vmin:
            vmin, vmax = float(power[finite].min()), float(power[finite].max())
        # Additionally cap dynamic range when plotting in dB. A long-tailed
        # noise floor can push vmin down by 80+ dB and wash everything out.
        if db_scale and np.isfinite(max_db_range) and (vmax - vmin) > max_db_range:
            vmin = vmax - float(max_db_range)
    else:
        vmin, vmax = None, None
