...
# CHANGE 1: Fixed argument parser (was "b0_mu" positional, now "--b0_mu" optional)
# CHANGE 2: Added --seed argument
def parse_args():
    parser = ArgumentParser()
    parser.add_argument("--R", type=int, default=100, help="Number of trials")
    parser.add_argument("--S", type=int, default=5, help="Number of units")
    parser.add_argument("--duration_sec", type=float, default=10.0, help="Duration of the simulation in seconds")
    parser.add_argument("--b0_mu", type=float, default=-2.0, help="Mean of the baseline coefficient (controls firing rate)")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility")
    parser.add_argument("--out", type=str, default="./data/sim_with_trials.pkl", help="Output path")
...
class TrialSimConfig:
    ...
    sigma_v_delta: np.ndarray = field(default_factory=lambda: np.array([5.0, 5.0, 5.0, 5.0], float))

    # Per-band complex observation noise for SIGNAL frequencies (same padding behavior)
    sigma_eps: np.ndarray = field(default_factory=lambda: np.array([5.0, 5.0, 5.0, 5.0], float))
    # Noise for non-signal dense frequencies
    sigma_eps_other: float = 10.0
...
def simulate_trials(cfg: TrialSimConfig, seed: int | None = None) -> dict:
    ...
    print(f"  b0_mu={cfg.b0_mu} (baseline firing rate parameter)")
...
if __name__ == "__main__":
    # CHANGE 3: Actually use parse_args() to override config values
    args = parse_args()
    ...
    print(f"Configuration: R={cfg.R}, S={cfg.S}, b0_mu={cfg.b0_mu}")
    
    data = simulate_trials(cfg, seed=args.seed)
